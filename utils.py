...
def save_masks(preds, save_path, mask_name, image_size, original_size, pad=None,  boxes=None, points=None):
    """
    Save the predicted mask image and optionally draw bounding boxes and keypoints.
    Args:
        predictions (torch.Tensor): Predicted mask tensor
        save_path (str): Save path
        mask_name (str): Mask file name
        image_size (int): Size of the input image
        original_size (tuple): Size of the original image (height, width)
        pad (tuple, optional): Size of image padding (top, left, bottom, right)
        boxes (torch.Tensor, optional): Bounding boxes tensor
        points (tuple, optional): Keypoints tuple (point_coords, point_labels)
    """
